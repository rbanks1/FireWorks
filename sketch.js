...
function mouseClicked(event) {
  ...
  // var itemFirework = new firework(x,y);
  ...
  drawFirework(){
    if (this.diameter < this.fireworkMaxDiameter) {
      ...
      for (let i = 0; i < this.numberOfParticles; i++) {
        ...
        point(this.x + this.diameter*Math.cos(radians(i*360/this.numberOfParticles)), this.y + this.diameter*Math.sin(radians(i*360/this.numberOfParticles)));
      }
     } 
  }
}
...
class stem {
  ...
  drawStem(){
    if (this.jumpX > 1){
      ...
      var itemFirework = new firework(this.currentX,this.currentY, this.colorStem);
      append(arrayFireworks, itemFirework);
